...
- **前台交付轨：** 优先复用已完成的816条真实Qwen校准响应，尽快形成明确标注为
  `preliminary / diagnostic`、可用于开学汇报的校准摘要、图表与限制说明；同时补齐真实
  vLLM事件适配和Phase 0B N=20最小冻结输入，争取尽快取得首个小规模真实动力学数据。
- **后台证据轨：** 不丢弃或混接既有证据，继续完成797条盲判、174项人工编码准备、
  一致性/裁决、gate report与Phase 0A-1封存。
- 两轨必须共享已批准规范但严格区分证据等级；初步/诊断结果不得冒充Paper 1正式主实验，
  后台证据链不得继续阻塞前台的非正式汇报交付。
- 当前优先级为“尽快取得可汇报真实数据”，避免重复重跑、全仓重复验证和非必要审计开发。
- **状态：** in_progress（用户于2026-09-20明确批准双轨推进）。
